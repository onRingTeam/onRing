...
**누구나 하나의 회의(스터디)에 함께 참여**할 수 있는 실시간 다국어 회의 애플리케이션입니다.

같은 회의방 안에서, 각자 편한 방식으로 대화합니다.

- **목소리로 참여하는 사람끼리는 WebRTC로 실제 음성 통화**를 하고,
- 말한 내용은 **온디바이스 음성인식(STT)** 을 거쳐 **자막**으로 모두에게 공유되며,
- **텍스트로 입력한 채팅**은 상대 기기에서 **음성(TTS)** 으로 재생됩니다.
- 참여자마다 언어(한/일/중/영)가 달라도 **온디바이스 번역**으로 서로의 발화를 각자 언어로 읽습니다.
- 회의가 끝나면 **AI(Gemini)가 회의록을 자동 요약**하고, 참여자마다 역할을 분류합니다.

> 핵심 설계 원칙: **텍스트는 STOMP, 음성은 WebRTC로 분리**
> 각 참여자는 "자기 마이크"만 STT 하고, 확정된 텍스트만 서버로 전송한다.

## 👥 팀원 소개

> 두 사람 모두 프런트·백엔드 전반에 참여했습니다.
...
- **음성 경로**: 음성 참여자 간에는 **WebRTC Mesh(P2P, 오디오 전용, 최대 6인)** 로 실제 목소리 통화. 시그널링(offer/answer/ICE)은 STOMP `/signal` 토픽을 재사용.
...
- **채팅 → TTS** — 텍스트로 입력한 채팅을 상대 기기에서 음성으로 재생
- **WebRTC 음성 통화** — 음성 참여자 간 자연스러운 오디오 통화 (Mesh, 오디오 전용)
